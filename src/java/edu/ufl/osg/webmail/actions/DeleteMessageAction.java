...
public class DeleteMessageAction extends FolderAction {
    ...
    public ActionForward execute(final ActionMapping mapping, final ActionForm form, final HttpServletRequest request, final HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            final Message nextMessage = ActionsUtil.fetchNextMessage(message, new DateSort());
            ...
            try {
                trashFolder = Util.getFolder(session, Constants.TRASH_FOLDER_FULLNAME);
                if (!deleteForever && (trashFolder != null // if we are in the trash folder, don't bother
                        && !Constants.TRASH_FOLDER_FULLNAME.equals(folder.getFullName()))) {

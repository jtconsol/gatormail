    ...
    private class DBProperties extends Properties {
        ...
        private void deleteKey(final String key) throws RuntimeException {
            ...
            try {
                ...
                try {
                    ...
                    //re.initCause(se); // TODO: Needs Java 1.4
                    throw re;
                }

                final int count = ps.executeUpdate();
                if (count > 1) {
                    logger.info("Deleted " + count + " rows for: " + getUserKey());
                }
        ...
        private void insertKeyValue(final String key, final String value) throws RuntimeException {
            if (key == null || value == null) {
                throw new NullPointerException("key and value must not be null. key: " + key + " value: " + value);
            }
            Connection connection = null;
            try {
                connection = connectionProvider.getConnection();
                ...
                final int count = ps.executeUpdate();
                if (count == 0) {
                    final RuntimeException re = new RuntimeException("Preferences insert failed for " + ps);
                    //re.initCause(se); // TODO: Needs Java 1.4
                    throw re;
                }

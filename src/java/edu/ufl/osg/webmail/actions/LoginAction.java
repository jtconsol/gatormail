...
//             session.setAttribute(Constants.MAIL_STORE_KEY, mailStore);
        ...
        updateSessionExpirationTimeout(session, prefs);
    ...
    /**
     * Updates the user's session timeout according to their preferences.
     *
     * @param user                the currently logged in user.
     * @param session             the {@link HttpSession} for that
     *                            <code>user</code>.
     */
    private void updateSessionExpirationTimeout(final HttpSession session, final Properties prefs) {
        final String maxInactiveMultiplier = prefs.getProperty("session.maxInactive.multiplier");
        if (maxInactiveMultiplier != null) {
            try {
                final int newMaxInactiveInterval = (int)(session.getMaxInactiveInterval() * Float.parseFloat(maxInactiveMultiplier));
    ...
    private static Folder getReservedFolder(final HttpSession session, final String folderName) throws MessagingException {
        ...
        } else // reserved folder does not exist - let's create it

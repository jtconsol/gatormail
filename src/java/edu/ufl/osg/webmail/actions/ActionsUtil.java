...
import edu.ufl.osg.webmail.beans.QuotaResourceBean;
...
final class ActionsUtil {
     ...
     *                                 some reason.
     */
    public static void checkSession(final HttpServletRequest request) throws SessionExpiredException, InvalidSessionException, NoSuchProviderException {
     ...
     * @see
     *                            #fetchMessage(org.apache.struts.action.ActionForm,
     *                            javax.mail.Folder,
    ...
    /**
     * Get the Quota List of QuotaResourceBeans
     */
    protected static List getQuotaList(final Folder folder) throws MessagingException {
        // Populate Quota info
        final List quotaList = new ArrayList();
        if (folder instanceof IMAPFolder) {
            final Quota[] quotas = ((IMAPFolder)folder).getQuota();
            for (int i = 0; i < quotas.length; i++) {
                final Quota.Resource[] resources = quotas[i].resources;
                final List resourceList = new ArrayList();
                for (int j = 0; j < resources.length; j++) {
                    resourceList.add(new QuotaResourceBean(resources[j].name, resources[j].usage, resources[j].limit));
                }
                quotaList.add(resourceList);
            }
        }
        logger.debug("calculated quotaList: " + quotaList.size() + " items");
        return quotaList;
    }
    ...
    private static void setFolderBeanList(final HttpSession session, final List list, final Folder rootFolder, final boolean includeReserved, final boolean includeUnsubscribed) throws MessagingException {
        ...
        try {
            ...
            if (isFolderIncluded(rootFolder, includeReserved) && rootFolder.exists()) {
    ...
    private static boolean isFolderIncluded(final Folder folder, final boolean includeReserved) {
        return (!includeReserved && Util.isReservedFolder(folder.getFullName())) ? false : true;
    }
    ...
    static {
        ...
        for (int i = 0; i < legalChars.length; i++) {
            logger.debug(String.valueOf(legalChars[i]));
        }
    }
    ...
    protected static boolean isLegalNewFolder(final String folderName, final String parentName, final ActionErrors errors, final HttpServletRequest request) throws MessagingException {
        ...
        for (int i = 0; i < length; i++) {
            if (Arrays.binarySearch(legalChars, tempName.charAt(i)) < 0) {
                logger.error("checked folder name. illegal character found: " + Arrays.binarySearch(legalChars, tempName.charAt(i)));
     ...
     *                             false if not.
     */
    protected static boolean isLegalDelete(final Folder folder, final ActionErrors errors) throws MessagingException {
        ...
        if (Util.isReservedFolder(deleteFolderName)) {
    ...
    protected static boolean appendMessage(final Message message, final Folder folder, final ActionErrors errors) throws MessagingException {
        ...
        try {
            ...
            logger.debug(".. appended message to Sent");
    ...
    protected static List copyMessages(final List messageList, final Folder folder, final Folder toFolder, final ActionErrors errors) throws MessagingException {
        ...
        return copyMessages(messages, folder, toFolder, errors);
    }
     ...
     * Returned folder is always open.
     */
    protected final static Folder createFolder(final Folder folder) throws MessagingException {
     ...
     */
    protected final static List buildMessageList(final Message[] messages) throws MessagingException {
        // Get the Messages in the folder
        final List messageList = new ArrayList(Arrays.asList(messages));

        // Remove any expunged messages, to prevent a NullPointerException
        final ListIterator it = messageList.listIterator();
        while (it.hasNext()) {

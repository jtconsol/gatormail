...
%>
  <td align="center"><c:forEach items="${message.flags.systemFlags}" var="flag"><%
...
--%>
   ...
   <c:when test="${column == 'to'}">
    <td>
     <c:forEach items="${wrappedMessage.to}" var="to" >
      <wm:write name="to" filter="false" formatKey="folder.view.email.format" formatClassKey="message.email.formatter"/>
     </c:forEach>
    </td>
   </c:when>

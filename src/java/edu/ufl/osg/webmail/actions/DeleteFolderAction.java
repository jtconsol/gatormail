...
public class DeleteFolderAction extends Action {
    ...
    public ActionForward execute(final ActionMapping mapping, final ActionForm form, final HttpServletRequest request, final HttpServletResponse response) throws Exception {
        ...
        Folder folder = null;
        try {
            folder = Util.getFolder(request.getSession(), deleteFolder);
            logger.debug("deleteFolder: " + folder);

            // check that this folder qualifies for deletion
            if (!ActionsUtil.isLegalDelete(folder, errors)) {
        ...
        // Populate list for folder table
        //final HttpSession session = request.getSession();
        //List folderBeanList = ActionsUtil.getFolderBeanList(session, folder);
